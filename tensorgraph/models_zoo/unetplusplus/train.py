...
def train(seq, X_ph, y_ph, X_train, y_train):
    y1_train_sb, y2_train_sb, y3_train_sb, y4_train_sb = seq.train_fprop(X_ph)
    y1_test_sb,  y2_test_sb,  y3_test_sb,  y4_test_sb  = seq.test_fprop(X_ph)
    train_cost_sb = entropy(y_ph, y1_train_sb) + entropy(y_ph, y2_train_sb) + \
                    entropy(y_ph, y3_train_sb) + entropy(y_ph, y4_train_sb)
    optimizer = tf.train.AdamOptimizer(0.0001)
    test_accu_sb = accuracy(y_ph, y1_test_sb) + accuracy(y_ph, y2_test_sb) + \
                   accuracy(y_ph, y3_test_sb) + accuracy(y_ph, y4_test_sb)
