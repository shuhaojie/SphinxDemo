...
def focal(ytrue, ypred, gamma=2.0, alpha=0.25):
    '''
    ...
    '''
    pos_weight = tf.where(label > 0, ytrue - ypred, tf.zeros_like(ypred))
    neg_weight = tf.where(label > 0, tf.zeros_like(ypred), ypred)

    ypred = tf.clip_by_value(ypred, epsilon, 1.0 - epsilon)
    cost_ = - alpha * (pos_weight ** gamma) * tf.log(ypred) \
                - (1 - alpha) * (neg_weight ** gamma) * tf.log(1-ypred)

    return tf.reduce_mean(cost_)
...
def tversky(ytrue, ypred, alpha=0.7, smooth=1.0):
    '''
    ...
    '''
    tp = tf.reduce_sum((ytrue)*(ypred), axis=(1,2))  # true positive
    fn  = tf.reduce_sum((ytrue)*(1-ypred), axis=(1,2))  # false negative
    fp  = tf.reduce_sum((1-ytrue)*(ypred), axis=(1,2))  # false positive

    loss_ = 1.0 - (tp + smooth) / (tp + alpha*fn + (1-alpha)*fp + smooth)
