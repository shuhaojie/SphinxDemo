...
class ImageResize(BaseLayer):

    @BaseLayer.init_name_scope
    def __init__(self, size, method='nearest_neighbor'):
        """
        Description:
            resizing image

        Args:
            size (tuple): tuple of 2 elements, (new_height, new_width)
            method (str): interpolation method and can be one of
                          'nearest_neighbour', 'bilinear' or 'bicubic'

        Returns:
            given state of shape [b, h, w, c], return
            shape of [b, new_height, new_width, c]
        """
        self.size = size
    ...
    def _train_fprop(self, state_below):
        ...
        Args:
            state_below: (b, h, w, c) 
        '''
        return tf.image.resize_images(state_below, self.size, method=self.method)
